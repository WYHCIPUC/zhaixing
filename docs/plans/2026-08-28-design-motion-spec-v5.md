...
## 0.5 场景叙事：书房的空间隐喻（2026-08-28 用户确认，全应用的叙事骨架）

**用户原话级需求**：打开项目＝走进书房；左侧功能＝书架上的一本本书；右侧＝书桌；点星穹＝深夜推开窗望向星空，看一条条思绪时隐时现。

**表达纪律**：隐喻靠**结构、比例、动效与命名**传达，**不靠拟物材质**——木纹／羊皮纸／衬线古籍感是已否决路线，禁止出现。书房是「Notion 排版的现代书房」。

| 场景 | 界面 | 现状 | 本 spec 落地 |
| --- | --- | --- | --- |
| 书房 | 亮暖外壳 | ✅ v4 令牌即书房底色 | 不变 |
| 书架上的书 | 左侧导航六项 | 普通列表＋琥珀激活 | 「书脊」化（下 ①） |
| 书桌 | 主内容区 | ✅ 白面＋panel 纸页感成立 | 留白节奏不动 |
| 窗 | 星穹画布 | ✅ 圆角＋white/10 边框＋大投影已是窗框 | 进场＝「推窗」（下 ③） |
| 时隐时现的思绪 | 连线／星云辉光 | 静态 | 「呼吸」（下 ④） |
| 翻阅一本书 | 打开书详情 | 渐进披露已定稿 | 不变 |

**① 书脊化侧栏**：每个导航项左侧一根 3px 竖向书脊条，颜色从莫兰迪书色盘为六个分区各固定一色（与星图用色同源——书是星的本源）；hover 书脊微亮；**激活态＝「书被抽出来」**：整项右移 3px＋书脊满高＋底色 surface-2＋文字 --text——**琥珀从侧栏激活态退出**，琥珀只留主按钮（单强调纪律回收）。切换零动画（高频层，§3.2）。

**② 桌面留白**：内容区维持白桌＋纸页卡片；空态文案保持现有书房语气（如「选一片星云，让星空自己长出第一篇文章」），新增文案沿用此声口。

**③ 推窗转场（星穹进场，招牌时刻）**：
- 进：壳内容 140ms 退场 → 画布「亮度升起」式进场：画布容器 `opacity 0→1` 500ms＋`filter: brightness(0.55→1)` 650ms `--ease-out`，亮星层先现、尘星层后现（engine 分层渐显），窗框恒在——不是页面跳转，是**开窗**。
- 会话内首次进星穹用完整版（≈650ms）；当次会话再进缩短为 250ms 短亮起（频率适配）；transition 天然可打断，随时点走。
- 出：反向「灯亮回来」200ms，同路径（纯 fade，无位移）。
- 实现载体：canvas 元素上的 CSS opacity/filter（GPU 合成），engine 无需重渲染。

**④ 思绪呼吸（画布内，克制的环境动效）**：
- 连线（twin 蓝／collision 红／manual 琥珀）：静态→每条线 opacity 以 6–10s 随机相位在 [0.35, 0.8] 间正弦起伏——时隐时现，永不同步。
- 星云辉光：保留 8s refresh，辉光半径加 ±2% 极慢呼吸。
- 偶发流星：每 25–60s 随机一颗细流星划过画布深处（约 800ms，不与 UI 抢戏）——「闪念」。
- 三者全部纳入 reduced-motion 停用清单（§7）；正弦调 opacity 是引擎内浮点运算，零额外合成层，帧预算不变。

**⑤ 书房声口**：界面文案自带书房语气已是事实；后续新文案（按钮／空态／错误）沿用，不引入系统腔。
...
### 4.1 全局壳
- **视图切换**（App.tsx:63-71）：现 180ms fade+y 基本合格。修订：进场 `y:10→0, opacity, 200ms --ease-out`；退场 `y:0→-6, opacity, 140ms --ease-out`（退场更快）；`AnimatePresence mode="wait"` 保留。**唯一例外：目标视图是星穹时，不走本条，改走 §0.5-③「推窗转场」。**
...
### 4.3 星穹 SkyView + 三抽屉
- **画布（starfield/engine.ts）**：深空观景窗是产品灵魂，**环境动效保留**（星尘/银河/辉光为身份性存在，归「Rare 高预算」）；但必须：① `prefers-reduced-motion` 时停在单帧（无闪烁/无漂移），② 帧预算 60fps，星星增量渲染，③ 之上浮层文字满足 vibrancy 对比。**进场走 §0.5-③ 推窗转场；连线/辉光/偶发流星的「思绪呼吸」按 §0.5-④ 实现。**
...
5. 星穹开幕·推窗转场（§0.5-③）：会话内首次 ≈650ms 亮度升起，之后 250ms；用户点名要的场景时刻，超出 300ms 规则的**已记录豁免**。
预算原则：这五处之外出现 bounce/长动画 = 违规。
...
| P1 基元 | 按钮/输入/弹层/抽屉统一到令牌；`useDrawerSpring`、`Stagger` 两个共享封装；cell-in/radar-in 重写；**§0.5-① 书脊化侧栏（含琥珀退出激活态）** | components 基元 | 抽屉 280/200 同边退场；起跳无 scale<0.9；侧栏激活=抽出的书 |
| P2 分面 | §4 各屏 pass + §6 修正清单清零；散参按映射表收敛；**§0.5-③ 推窗转场** | 各 view/components diff | 慢放 2–5× 复查无跳变；隔日复看；推窗可打断 |
| P3 delight | §4.5 五个稀有时刻 + **§0.5-④ 思绪呼吸（连线呼吸/辉光呼吸/偶发流星）** + 画布 reduce-motion 单帧 + 移动端手势（速度甩出/rubber-band/44px） | 稀有时刻动效 | 移动端真机手感；reduce-motion 全量走查；帧预算 60fps 不掉 |
