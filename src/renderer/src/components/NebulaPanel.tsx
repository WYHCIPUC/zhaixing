...
export default function NebulaPanel({
  ...
  return (
    <motion.div
      ...
      exit={{ opacity: 0, y: 20 }}
      className="absolute inset-x-0 bottom-0 z-30 max-h-[62%] overflow-hidden border-t border-[var(--line)] bg-[#0b1120f8]"
    >
      ...
      <div className="max-h-[calc(62vh-56px)] overflow-y-auto px-8 py-4">
        ...
        <div className="grid grid-cols-[repeat(auto-fill,minmax(300px,1fr))] gap-3">
          {stars.map((s) => (
            <button
              key={s.id}
              className="panel block p-3 text-left transition-colors hover:border-[rgba(125,211,252,0.4)]"
