      ...
      <header className="mb-5 flex items-center gap-4">
        ...
        <div className="ml-auto flex rounded-lg border border-[var(--line)] p-0.5">
          {(
            ...
            <button
              key={k}
              className={`rounded-md px-3 py-1 text-[12.5px] ${mode === k ? 'bg-[rgba(217,122,30,0.15)] text-[var(--accent)]' : 'text-[var(--text-dim)]'}`}
              onClick={() => setMode(k)}
            >
              {label}
            </button>
                  ...
                  <button
                    ...
                    className={`block w-full rounded-lg border px-3 py-2 text-left text-[12.5px] transition-colors ${
                      activeNebula?.id === n.id
                        ? 'border-[rgba(217,122,30,0.5)] text-[var(--text)]'
                  ...
                  {articles.map((a) => (
                    <button
                      ...
                      className={`block w-full rounded-lg border px-3 py-2 text-left text-[12px] ${
                        active?.id === a.id ? 'border-[rgba(217,122,30,0.5)]' : 'border-[var(--line)]'
                      }`}
                      onClick={() => {
                        setActive(a)
                        setDraft(a.content_md)
